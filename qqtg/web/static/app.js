/* QQTG Bridge admin panel – dependency-free single page app. */
(function () {
  "use strict";
  ...
  const PLAT = { qq: "QQ", telegram: "Telegram" };
  const DIR = { both: "双向", qq_to_tg: "QQ → TG", tg_to_qq: "TG → QQ" };
  const ARROW = { both: "⇄", qq_to_tg: "→", tg_to_qq: "←" };
  ...
  const state = { session: null, csrf: null, route: "", timers: [] };
  ...
  function shell(content, active) {
    ...
    return `<div class="layout">
      <aside class="sidebar">
        <div class="brand"><div class="logo">⇄</div><div>QQTG Bridge</div></div>
        <nav class="nav">${NAV.map(([h, n, i]) => `<a href="${h}" class="${active === h ? "active" : ""}"><span class="ico">${i}</span>${n}${h === "#/chats" && state.pendingCount ? `<span class="badge warn">${state.pendingCount}</span>` : ""}</a>`).join("")}</nav>
        <div class="foot"><div class="user">${esc(u.username)} <span class="badge">${u.role}</span></div><div>v${esc(state.version || "")} · <a href="#" data-logout>退出</a></div></div>
      </aside>
  ...
  // ------------------------------------------------------------ auth views
  function viewSetup() {
    mount(`<div class="auth"><div class="card">
      <div class="brand"><div class="logo">⇄</div>QQ ↔ Telegram Bridge</div>
      <div class="sub">欢迎使用！请先创建管理员 (Owner) 账号</div>
      <form id="f">
        ...
        <button class="btn primary" style="width:100%;justify-content:center">创建并进入面板</button>
      </form></div></div>`);
    $("#f").onsubmit = async (e) => {
      e.preventDefault();
  ...
  function viewLogin() {
    mount(`<div class="auth"><div class="card">
      ...
      <form id="f">
        ...
        <button class="btn primary" style="width:100%;justify-content:center">登录</button>
      </form></div></div>`);
    $("#f").onsubmit = async (e) => {
      e.preventDefault();
      const fd = Object.fromEntries(new FormData(e.target).entries());
  ...
  async function viewDashboard() {
    ...
    const conns = { qq: h.components.qq.ok, tg: h.components.telegram.ok };
    const hasQQ = !!(h.adapters.qq), hasTG = !!(h.adapters.telegram);
    const enabled = d.bridges.filter((b) => b.enabled).length;
    const onboardStep = !hasQQ ? 1 : !hasTG ? 2 : d.bridges.length === 0 ? 3 : enabled === 0 ? 3 : 4;
    const onboard = onboardStep < 4 ? `<div class="card"><div class="card-head"><h2>快速开始</h2><span class="muted small">按顺序完成 4 步即可开始桥接</span></div>
      <div class="onboard">
        ${[["连接 QQ", "#/connections"], ["连接 Telegram", "#/connections"], ["创建桥接", "#/bridges/new"], ["完成", "#/"]].map(([n, href], i) => `<a class="ob ${i + 1 < onboardStep ? "done" : i + 1 === onboardStep ? "active" : ""}" href="${href}"><div class="n">${i + 1 < onboardStep ? "✓" : i + 1}</div><div>${n}</div></a>`).join("")}
      </div></div>` : "";
    ...
    mount(shell(`
      ...
      <div class="grid cols-4" style="margin-top:16px">
        <div class="card stat"><div class="label">今日消息</div><div class="value">${s.sent}</div><div class="delta">QQ→TG ${s.qq_to_tg} · TG→QQ ${s.tg_to_qq}</div></div>
        <div class="card stat"><div class="label">今日媒体</div><div class="value">${Object.entries(s.kinds).filter(([k]) => k !== "text").reduce((a, [, v]) => a + v, 0)}</div><div class="delta">${Object.entries(s.kinds).filter(([k]) => k !== "text").map(([k, v]) => `${MEDIA_LABEL[k] || k} ${v}`).join(" · ") || "—"}</div></div>
        <div class="card stat"><div class="label">今日失败</div><div class="value" style="color:${s.failed ? "var(--bad)" : "inherit"}">${s.failed}</div><div class="delta">队列 ${h.queue.depth} · 等待重试 ${h.queue.retries_waiting}</div></div>
        <div class="card stat"><div class="label">桥接</div><div class="value">${enabled}<span class="muted" style="font-size:14px;font-weight:500"> / ${d.bridges.length}</span></div><div class="delta">已启用 / 总数</div></div>
      </div>
      <div class="grid cols-2" style="margin-top:16px">
        <div class="card"><div class="card-head"><h2>System Health</h2><span class="muted small">每 10 秒刷新</span></div>
          ...
          <div style="margin-top:14px" class="row">
            <div><span class="dot ${conns.qq ? "ok" : "bad"}"></span> QQ ${hasQQ ? (h.adapters.qq.self_name ? esc(h.adapters.qq.self_name) + " (" + esc(h.adapters.qq.self_id) + ")" : "") : "未配置"} ${hasQQ && !conns.qq ? `<span class="badge bad">${esc(h.adapters.qq.last_error || "离线")}</span>` : ""}</div>
            <div><span class="dot ${conns.tg ? "ok" : "bad"}"></span> Telegram ${hasTG ? (h.adapters.telegram.self_name ? "@" + esc(h.adapters.telegram.self_name) : "") : "未配置"} ${hasTG && !conns.tg ? `<span class="badge bad">${esc(h.adapters.telegram.last_error || "离线")}</span>` : ""}</div>
          </div>
        </div>
        <div class="card"><div class="card-head"><h2>Issues</h2><a href="#/messages?status=failed" class="small">查看详情</a></div>
          ...
          ${d.pending_chats.length ? `<div class="alert warn" style="margin-top:12px">有 ${d.pending_chats.length} 个群组在等待授权 → <a href="#/chats">前往处理</a></div>` : ""}
          <div style="margin-top:12px"><div class="muted small" style="margin-bottom:6px">近 7 天消息量</div><div class="sparkline">${week.map((w) => `<div class="bar" title="${w.day}: ${w.sent} 成功 / ${w.failed} 失败" style="height:100%"><i style="height:${Math.round(((w.sent || 0) / max) * 100)}%"></i></div>`).join("") || '<span class="muted small">暂无数据</span>'}</div></div>
        </div>
      </div>
      <div class="card-head" style="margin:22px 0 10px"><h2>Bridges</h2><a href="#/bridges" class="small">管理全部</a></div>
      ${d.bridges.length ? `<div class="grid cols-2">${d.bridges.map(bridgeCard).join("")}</div>` : `<div class="card empty">还没有桥接。<a href="#/bridges/new">创建第一条桥接</a></div>`}
    `, "#/"));
  ...
  function errorLabel(code) {
    return { FILE_TOO_LARGE: "文件过大", CHAT_FORBIDDEN: "机器人被移出或无权限", NO_PERMISSION: "TG 权限不足", CHAT_NOT_FOUND: "找不到目标群", MUTED: "QQ 被禁言",
      NOT_IN_GROUP: "机器人不在 QQ 群", TARGET_OFFLINE: "目标平台未连接", NETWORK: "网络错误", RATE_LIMIT: "限流", MEDIA_CONVERT_FAILED: "媒体转换失败", MEDIA_TIMEOUT: "媒体处理超时",
      FFMPEG_MISSING: "缺少 FFmpeg", DOWNLOAD_FAILED: "媒体下载失败", RISK_CONTROL: "QQ 风控", QQ_API_FAILED: "QQ 接口失败", TIMEOUT: "接口超时", INTERNAL: "内部错误", EMPTY: "空消息", BAD_MEDIA: "媒体不被接受" }[code] || (code || "未知错误");
  }

  function bridgeCard(b) {
    const warns = b.warnings || [];
    const st = !b.enabled ? ["○ 已停用", ""] : warns.length ? ["● 运行中（有告警）", "warn"] : ["● 正常运行", "ok"];
    return `<div class="card bridge-card">
      <div class="title"><h3>${esc(b.name)}</h3><span class="badge ${st[1]}">${st[0]}</span></div>
      <div class="pair"><div class="side"><div class="plat">QQ 群</div><div class="name" title="${esc(b.qq_chat)}">${esc(b.qq_title)}</div></div><div class="arrow">${ARROW[b.direction]}</div><div class="side" style="text-align:right"><div class="plat">TELEGRAM</div><div class="name" title="${esc(b.tg_chat)}">${esc(b.tg_title)}</div></div></div>
      <div class="meta"><span>今日消息 ${b.today.sent}</span><span>今日媒体 ${b.today.media}</span>${b.today.failed ? `<span style="color:var(--bad)">失败 ${b.today.failed}</span>` : ""}<span>${DIR[b.direction]}</span></div>
  ...
  function showTestResult(results) {
    const rows = Object.entries(results).map(([k, v]) => `<div class="ci ${v.ok ? "ok" : "bad"}"><span class="mark">${v.ok ? "✓" : "✗"}</span><span>${DIR[k]}</span><span class="muted">${esc(v.error || "")}</span></div>`).join("");
  ...
  async function runDiagnose() {
    ...
    try {
      ...
      m.el.innerHTML = `<h3>一键诊断</h3><div class="checklist">${r.checks.map((c) => `<div class="ci ${c.status === "PASS" ? "ok" : c.status === "WARN" ? "warn" : "bad"}"><span class="mark">${c.status === "PASS" ? "✓" : c.status === "WARN" ? "!" : "✗"}</span><span style="min-width:200px">${esc(c.name)}</span><span class="mono" style="min-width:44px">${c.status}</span><span class="muted small">${esc(c.detail || "")}</span></div>`).join("")}</div><div class="foot"><button class="btn primary" data-x>关闭</button></div>`;
      ...
      <div class="grid cols-2">
        ...
        <div class="card">
          ...
          ${owner ? `<form id="ftg">
            ...
            <div class="field"><label>Bot API 地址（可选）</label><input class="input mono" name="api_base" value="${esc(tg.config ? tg.config.api_base : "")}" placeholder="https://api.telegram.org（自建 Bot API Server 可突破 50MB 限制）"></div>
            <div class="actions"><button class="btn primary">验证并保存</button><button class="btn" type="button" data-tgtest>仅验证 Token</button>${tg.configured ? `<button class="btn" type="button" data-restart="telegram">重新连接</button><button class="btn danger" type="button" data-del="telegram">移除</button>` : ""}</div>
          </form>` : ""}
          <div class="muted small" style="margin-top:14px">保存后，请把机器人添加到目标群组并发送 <code>/bridge</code>，群组会自动出现在「群组」页面。</div>
        </div>
      </div>
    `, "#/connections"));
    const fqq = $("#fqq");
  ...
  async function viewChats() {
    ...
    const section = (platform) => {
      const list = chats.filter((c) => c.platform === platform);
      return `<div class="card"><div class="card-head"><h2>${PLAT[platform]} 群组 <span class="muted small">${list.length}</span></h2></div>
        ...
        ${list.map((c) => `<tr>
          ...
          </div></td></tr>`).join("")}</tbody></table></div>` : `<div class="empty">尚未发现 ${PLAT[platform]} 群组。${platform === "qq" ? "请确认 QQ 已连接；连接后会自动读取群列表。" : "请把机器人加入群组，并在群里发送 /bridge。"}</div>`}
      </div>`;
    };
    mount(shell(`
      ...
      ${section("qq")}<div style="height:16px"></div>${section("telegram")}
    `, "#/chats"));
    ...
    const rf = $("[data-refresh]"); if (rf) rf.onclick = async () => { rf.disabled = true; try { const r = await post("/chats/refresh"); toast(`已刷新：${Object.entries(r.counts).map(([k, v]) => `${PLAT[k]} ${v}`).join("，") || "无已连接平台"}`, "ok"); viewChats(); } catch (e) { toast(e.message, "bad"); rf.disabled = false; } };
    const mn = $("[data-manual]"); if (mn) mn.onclick = () => {
      const m = modal(`<h3>手动添加群组</h3><form id="fm"><div class="field"><label>平台</label><select class="input" name="platform"><option value="qq">QQ</option><option value="telegram">Telegram</option></select></div><div class="field"><label>群 ID</label><input class="input mono" name="chat_id" required placeholder="QQ 群号 / Telegram chat id（如 -1001234567890）"></div><div class="field"><label>名称（可选）</label><input class="input" name="title"></div><div class="foot"><button class="btn" type="button" data-x>取消</button><button class="btn primary">添加</button></div></form>`);
      $("[data-x]", m.el).onclick = m.close;
  ...
  async function viewBridges() {
    ...
    mount(shell(`
      <div class="page-head"><div><h1>桥接</h1><div class="sub">一条桥接 = 一个 QQ 群 ↔ 一个 Telegram 群。一个机器人可以挂任意多条桥接。</div></div>
        <div class="actions">${can("owner") ? `<a class="btn primary" href="#/bridges/new">＋ 创建桥接</a>` : ""}</div></div>
      ${r.bridges.length ? `<div class="grid cols-2">${r.bridges.map(bridgeCard).join("")}</div>` : `<div class="card empty">还没有桥接。<a href="#/bridges/new">创建第一条桥接</a></div>`}
    `, "#/bridges"));
  ...
  async function viewBridgeNew() {
    const r = await get("/chats");
    const qqChats = r.chats.filter((c) => c.platform === "qq" && !["rejected", "disabled"].includes(c.status));
    const tgChats = r.chats.filter((c) => c.platform === "telegram" && !["rejected", "disabled"].includes(c.status));
    const w = { step: 1, qq: null, tg: null, name: "", direction: "both", media: Object.fromEntries(Object.keys(MEDIA_LABEL).map((k) => [k, true])), reply_sync: true, recall_sync: false, edit_sync: false, event_sync: false, display_mode: "", bridgeId: null, verify: null, test: null };
    const steps = ["选择 QQ 群", "选择 Telegram 群", "桥接选项", "检查与测试", "完成"];
    const choiceList = (list, sel, key) => list.length ? `<div class="choice-list">${list.map((c) => `<div class="choice ${sel === c.id ? "selected" : ""}" data-pick="${c.id}"><div class="radio"></div><div class="info"><div class="t">${esc(c.title)}</div><div class="s mono">${esc(c.chat_id)}${c.member_count != null ? " · " + c.member_count + " 人" : ""} · ${esc(c.status_label)}</div></div>${c.bridges.length ? `<span class="badge info">已有 ${c.bridges.length} 条桥接</span>` : ""}</div>`).join("")}</div>` : `<div class="empty">没有可用的 ${key === "qq" ? "QQ" : "Telegram"} 群组。${key === "qq" ? '请先在「连接」页面连接 QQ。' : '请把机器人拉进群并发送 /bridge，或在「群组」页面手动添加。'}</div>`;
    const draw = () => {
      const q = qqChats.find((c) => c.id === w.qq), t = tgChats.find((c) => c.id === w.tg);
      let body = "";
      if (w.step === 1) body = `<h2 style="margin-bottom:12px">选择 QQ 群</h2>${choiceList(qqChats, w.qq, "qq")}<div class="actions" style="margin-top:16px;justify-content:flex-end"><button class="btn primary" data-next ${w.qq ? "" : "disabled"}>下一步</button></div>`;
      else if (w.step === 2) body = `<h2 style="margin-bottom:12px">选择 Telegram 群</h2>${choiceList(tgChats, w.tg, "tg")}<div class="actions" style="margin-top:16px;justify-content:space-between"><button class="btn" data-prev>上一步</button><button class="btn primary" data-next ${w.tg ? "" : "disabled"}>下一步</button></div>`;
      else if (w.step === 3) body = `<h2 style="margin-bottom:12px">桥接选项</h2>
        <div class="pair" style="display:grid;grid-template-columns:1fr auto 1fr;align-items:center;gap:10px;background:var(--bg);border-radius:10px;padding:12px 14px;margin-bottom:16px"><div><div class="muted small">QQ 群</div><b>${esc(q.title)}</b></div><div style="font-size:20px;color:var(--primary)">⇄</div><div style="text-align:right"><div class="muted small">TELEGRAM</div><b>${esc(t.title)}</b></div></div>
        <div class="field"><label>名称</label><input class="input" id="w-name" value="${esc(w.name || (q.title + " ↔ " + t.title))}"></div>
        <div class="field"><label>消息方向</label><div class="checks">${Object.entries(DIR).map(([k, v]) => `<label class="check"><input type="radio" name="dir" value="${k}" ${w.direction === k ? "checked" : ""}> ${v}</label>`).join("")}</div></div>
        ...
        <div class="field"><label>显示模式</label><select class="input" id="w-mode"><option value="">跟随全局设置</option><option value="simple">简洁（张三 / 内容）</option><option value="standard">标准（QQ · 张三 / 内容）</option><option value="full">完整（含群名与时间）</option></select></div>
        <div class="field"><label>高级</label><div class="checks">
          ...
          <label class="check"><input type="checkbox" data-opt="recall_sync" ${w.recall_sync ? "checked" : ""}> 撤回同步（QQ→TG）</label>
          <label class="check"><input type="checkbox" data-opt="edit_sync" ${w.edit_sync ? "checked" : ""}> 编辑同步（TG→QQ）</label>
        ...
        const v = w.verify;
        const side = (label, s) => `<div><h3 style="margin-bottom:8px">${label} <span class="badge ${s.ok ? "ok" : "bad"}">${s.ok ? "条件满足" : "有问题"}</span></h3>${s.present === false ? `<div class="alert bad">机器人不在该群</div>` : ""}${s.reason ? `<div class="alert warn">${esc(s.reason)}</div>` : ""}<div class="checklist">${Object.entries(s.checks || {}).filter(([k]) => k !== "delete_messages").map(([k, ok]) => `<div class="ci ${ok ? "ok" : "bad"}"><span class="mark">${ok ? "✓" : "✗"}</span>${esc(PERM_LABEL[k] || k)}</div>`).join("") || '<div class="muted small">无详细信息</div>'}</div></div>`;
        body = `<h2 style="margin-bottom:12px">检查与测试</h2>${v ? `<div class="grid cols-2">${side("QQ 群", v.qq)}${side("Telegram 群", v.tg)}</div>${v.ok ? `<div class="alert ok" style="margin-top:14px">✓ 桥接条件满足</div>` : `<div class="alert warn" style="margin-top:14px">部分检查未通过。你仍然可以启用桥接，但相关消息可能发送失败。</div>`}` : `<div class="muted"><span class="spin"></span> 正在检查…</div>`}
          ${w.test ? `<h3 style="margin:16px 0 8px">测试消息</h3><div class="checklist">${Object.entries(w.test).map(([k, r]) => `<div class="ci ${r.ok ? "ok" : "bad"}"><span class="mark">${r.ok ? "✓" : "✗"}</span>${DIR[k]}<span class="muted small">${esc(r.error || "")}</span></div>`).join("")}</div>` : ""}
      ...
      } else body = `<div style="text-align:center;padding:30px 0"><div style="font-size:44px">🎉</div><h2 style="margin:10px 0 6px">桥接已${w.enabledNow ? "启用" : "创建"}</h2><div class="muted">${esc(w.name)}${w.enabledNow ? " 正在运行，消息将开始双向同步。" : " 已保存但未启用，可随时在桥接页面启用。"}</div><div class="actions" style="justify-content:center;margin-top:20px"><a class="btn primary" href="#/bridges/${w.bridgeId}">管理桥接</a><a class="btn" href="#/bridges">返回列表</a></div></div>`;
      mount(shell(`<div class="page-head"><div><h1>创建桥接</h1></div><a class="btn ghost" href="#/bridges">取消</a></div>
        ...
        <div class="card">${body}</div>`, "#/bridges"));
      $$("[data-pick]").forEach((el) => el.onclick = () => { const id = parseInt(el.dataset.pick, 10); if (w.step === 1) w.qq = id; else w.tg = id; draw(); });
      ...
      const cr = $("[data-create]"); if (cr) cr.onclick = async () => {
        ...
        const ok = await confirmDialog("确认创建桥接？", `<dl class="kv"><dt>QQ</dt><dd>${esc(q.title)}</dd><dt>Telegram</dt><dd>${esc(t.title)}</dd><dt>消息方向</dt><dd>${DIR[w.direction]}</dd><dt>媒体</dt><dd>${Object.values(w.media).every(Boolean) ? "全部" : Object.entries(w.media).filter(([, v]) => v).map(([k]) => MEDIA_LABEL[k]).join("、")}</dd></dl><div class="muted small" style="margin-top:10px">创建后会先检查双方权限并可发送测试消息，确认无误后再启用。</div>`, "确认并检查");
        if (!ok) return;
        cr.disabled = true;
        try {
          const res = await post("/bridges", { name: w.name, qq_chat_id: w.qq, tg_chat_id: w.tg, direction: w.direction, enabled: false, options: { media: w.media, reply_sync: w.reply_sync, recall_sync: w.recall_sync, edit_sync: w.edit_sync, event_sync: w.event_sync, display_mode: w.display_mode || null } });
  ...
  async function viewBridgeDetail(id) {
    ...
    mount(shell(`
      <div class="page-head"><div><a href="#/bridges" class="small">← 桥接列表</a><h1>${esc(b.name)}</h1><div class="sub">#${b.id} · 创建于 ${fmtTime(b.created_at)}</div></div>
      ...
      <div class="grid cols-2">
        <div class="card">
          ...
          <div class="pair" style="display:grid;grid-template-columns:1fr auto 1fr;align-items:center;gap:10px;background:var(--bg);border-radius:10px;padding:12px 14px"><div><div class="muted small">来源 · QQ</div><b>${esc(b.qq_title)}</b><div class="mono muted small">${esc(b.qq_chat)}</div><span class="badge ${STATUS_CLS[b.qq_status] || ""}">${esc(b.qq_status)}</span></div><div style="font-size:22px;color:var(--primary)">${ARROW[b.direction]}</div><div style="text-align:right"><div class="muted small">目标 · Telegram</div><b>${esc(b.tg_title)}</b><div class="mono muted small">${esc(b.tg_chat)}</div><span class="badge ${STATUS_CLS[b.tg_status] || ""}">${esc(b.tg_status)}</span></div></div>
          <form id="fb" style="margin-top:16px">
            <div class="field"><label>名称</label><input class="input" name="name" value="${esc(b.name)}" ${dis}></div>
            <div class="field"><label>方向</label><div class="checks">${Object.entries(DIR).map(([k, v]) => `<label class="check"><input type="radio" name="direction" value="${k}" ${b.direction === k ? "checked" : ""} ${dis}> ${v}</label>`).join("")}</div></div>
            ...
            <div class="field"><label>高级</label>
              ${[["reply_sync", "回复同步", "跨平台保留回复关系"], ["recall_sync", "撤回同步", "QQ 撤回后删除 Telegram 上的副本（需机器人有删除权限）"], ["edit_sync", "编辑同步", "Telegram 编辑消息后向 QQ 发送 [消息已修改]"], ["event_sync", "群事件同步", "入群 / 退群提示（默认关闭以减少噪音）"]].map(([k, n, d]) => `<div class="toggle-row"><div><div>${n}</div><div class="desc">${d}</div></div><label class="switch"><input type="checkbox" data-opt="${k}" ${o[k] ? "checked" : ""} ${dis}><span class="track"></span></label></div>`).join("")}
            </div>
            ${owner ? `<button class="btn primary">保存</button>` : ""}
          </form>
        </div>
        <div>
          <div class="card"><div class="card-head"><h2>今日统计</h2></div><div class="grid cols-3"><div class="stat"><div class="label">消息</div><div class="value">${b.today.sent}</div></div><div class="stat"><div class="label">媒体</div><div class="value">${b.today.media}</div></div><div class="stat"><div class="label">失败</div><div class="value" style="color:${b.today.failed ? "var(--bad)" : "inherit"}">${b.today.failed}</div></div></div><div class="muted small">QQ → TG ${b.today.qq_to_tg} · TG → QQ ${b.today.tg_to_qq}</div></div>
          <div class="card"><div class="card-head"><h2>最近消息</h2><a class="small" href="#/messages?bridge_id=${b.id}">全部</a></div><div id="recent"><span class="spin"></span></div></div>
        </div>
      </div>
    `, "#/bridges"));
    const fb = $("#fb");
    ...
    try { const m = await get(`/messages?bridge_id=${id}&limit=15`); $("#recent").innerHTML = m.messages.length ? `<table>${m.messages.map((x) => `<tr><td class="muted small">${fmtTime(x.created_at).slice(5)}</td><td>${x.direction === "qq_to_tg" ? "QQ→TG" : "TG→QQ"}</td><td>${esc(x.source_user_name || "")}</td><td class="muted">${esc(x.summary)}</td><td><span class="badge ${MSG_STATUS[x.status][1]}">${MSG_STATUS[x.status][0]}</span></td></tr>`).join("")}</table>` : `<div class="empty">暂无消息</div>`; } catch (e) { }
  }
  ...
  async function viewMessages(query) {
    ...
    mount(shell(`
      ...
      <div class="card"><div class="table-wrap">${r.messages.length ? `<table><thead><tr><th>#</th><th>时间</th><th>方向</th><th>桥接</th><th>发送者</th><th>内容</th><th>类型</th><th>状态</th><th>耗时</th><th></th></tr></thead><tbody>
        ${r.messages.map((m) => `<tr class="clickable" data-row="${m.id}"><td class="muted">${m.id}</td><td class="muted small">${fmtTime(m.created_at)}</td><td>${m.direction === "qq_to_tg" ? "QQ → TG" : "TG → QQ"}</td><td>${esc(bname[m.bridge_id] || "-")}</td><td>${esc(m.source_user_name || "")}</td><td class="muted" style="max-width:280px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${esc(m.summary)}</td><td>${MEDIA_LABEL[m.kind] || m.kind}</td><td><span class="badge ${MSG_STATUS[m.status][1]}" title="${esc(m.error || "")}">${MSG_STATUS[m.status][0]}</span>${m.error ? `<div class="small" style="color:var(--bad);max-width:200px">${esc(errorLabel(m.error_code))}</div>` : ""}</td><td class="muted small">${m.duration_ms != null ? (m.duration_ms / 1000).toFixed(2) + "s" : "-"}</td><td>${can("admin") && (m.status === "dead") ? `<button class="btn sm" data-retry="${m.id}">重试</button>` : ""}</td></tr>
        <tr data-detail="${m.id}" style="display:none"><td colspan="10"><div class="timeline">${(m.steps || []).map((s) => `<div class="tl"><span class="muted">+${s.t}s</span><span class="${s.ok ? "ok" : "bad"}">${esc(s.name)}</span><span class="muted">${esc(s.detail || "")}</span></div>`).join("") || '<span class="muted small">无处理记录</span>'}${m.error ? `<div style="margin-top:6px"><b>技术详情：</b><code>${esc(m.error_code || "")}</code> ${esc(m.error)}</div>` : ""}<div class="muted small" style="margin-top:4px">源消息 ${esc(m.source_platform)} ${esc(m.source_chat_id)}/${esc(m.source_message_id)} · 尝试 ${m.attempts} 次</div></div></td></tr>`).join("")}</tbody></table>` : `<div class="empty">没有符合条件的消息</div>`}</div></div>
    `, "#/messages"));
  ...
  async function viewLogs(query) {
    ...
    mount(shell(`
      ...
      <div class="card"><div class="tabs">${cats.map(([k, v]) => `<div class="tab ${cat === k ? "active" : ""}" data-cat="${k}">${v}</div>`).join("")}</div>
  ...
  async function viewSettings() {
    ...
    mount(shell(`
      ...
      <div class="grid cols-2">
        <div>
          <form id="fs" class="card"><div class="card-head"><h2>设置</h2>${owner ? `<button class="btn primary sm">保存</button>` : ""}</div>
            ${groups.map(([g, keys]) => `<h3 style="margin:14px 0 4px;color:var(--muted);font-size:12px;text-transform:uppercase;letter-spacing:.05em">${g}</h3>${keys.map(field).join("")}`).join("")}
          </form>
        </div>
        <div>
          ${owner ? `<div class="card"><div class="card-head"><h2>用户与权限</h2><button class="btn sm" data-adduser>＋ 添加</button></div>
          ...
          ${owner ? `<div class="card"><div class="card-head"><h2>备份与恢复</h2></div>
          ...
          <div class="card"><div class="card-head"><h2>关于</h2></div><dl class="kv"><dt>版本</dt><dd>${esc(sys.version)}</dd><dt>Python</dt><dd>${esc(sys.python)}</dd><dt>FFmpeg</dt><dd class="mono">${esc(sys.ffmpeg || "未安装")}</dd><dt>安装目录</dt><dd class="mono">${esc(sys.home)}</dd><dt>服务</dt><dd class="mono">systemctl status qqtg-bridge</dd><dt>更新</dt><dd class="mono">qqtg-install update</dd></dl></div>
        </div>
      </div>
    `, "#/settings"));
    $("[data-diag]").onclick = runDiagnose;
  ...
  async function loadSession() {
    ...
    state.csrf = s.csrf;
    document.title = s.title || "QQ ↔ Telegram Bridge";
  }
